...
const CreativeCenter: React.FC<CreativeCenterProps> = ({ onPlaySong }) => {
  ...
  return (
    <div className="flex-1 overflow-y-auto px-6 pb-40 pt-10 bg-slate-950 custom-scrollbar relative">
      ...
      <header className="flex items-center justify-between mb-16 max-w-7xl mx-auto">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 font-bold tracking-[0.3em] uppercase mt-1">实时同步全网创作动态</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-[1fr_1.1fr_1fr] items-end gap-10 mb-28 max-w-7xl mx-auto px-4">
          ...
          {USER_CREATIONS.map((song, idx) => {
            ...
            return (
              <div 
                ...
                className={`grid grid-cols-[60px_40px_1fr_100px_120px] items-center px-8 py-5 rounded-2xl transition-all group border border-transparent ${hoveredId === song.id ? 'bg-white/5 border-white/5 shadow-xl translate-x-1' : 'hover:bg-white/[0.02]'}`}
              >
                ...
                <div className="flex items-center gap-5 min-w-0 pr-6">
                  ...
                  <div className="flex flex-col min-w-0 truncate">
                    ...
                    <span className="text-xs text-slate-500 font-bold group-hover:text-slate-400 transition-colors">作者：{song.artist}</span>
                  </div>
                </div>

                <div className="text-xs font-mono font-bold text-slate-700 group-hover:text-emerald-500 text-center">
                  {song.duration || '04:59'}
                </div>
